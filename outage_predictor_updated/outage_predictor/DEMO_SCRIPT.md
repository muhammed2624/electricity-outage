...
"The app is upfront about its limits too — for dates beyond the model's
training window, it labels this a Seasonal Risk Estimate rather than
faking a forecast. That's just the risk model's window, though — the
street search itself covers about 7,900 real streets parsed straight out
of NERC's own feeder filings, and the live Band data next to it stays
current no matter what date you pick. That distinction is stated right in
the app, not buried in a report."
...
- If you're being scored on the "nationwide" and "current data" points
  specifically, make sure the Band panel stays on screen long enough to
  actually read, and click open the "Data source & verification" expander
  underneath it to show the NERC citation — that single screen is your
  answer to "the data ends in 2022."
