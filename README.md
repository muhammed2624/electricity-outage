...
- `src/build_registry.py` — builds `street_grid_registry.csv`, the
  street/area to DisCo lookup covering all 11 DisCo territories.
...
Every area/DisCo pairing in the street registry is a real, publicly
documented DisCo franchise assignment — but the registry is a representative
seed (6-17 areas per DisCo), not an exhaustive street-by-street census.
Feeder codes and service bands shown in the app are explicitly marked
estimated, since no public feeder-level registry exists. The training data
...
because of a monthly-vs-quarterly granularity mismatch. Full detail in
`DATA_NOTES.md`.
...
Target: Streamlit Community Cloud, connected via GitHub. Main file path:
`outage_predictor/app/app.py` (relative to your repo root — adjust if your
repo's folder structure differs from this zip's).
